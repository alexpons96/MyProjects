 ...
 * amb les atributs necessaris per a transmitir informació i buscar informacio.
 */
public class ServerDedicat extends Thread {    // aixo més endavant heretarà de thread, i el podrem fer servir durant el joc per a les comandes cap al server
    ...
    public ServerDedicat(Socket socket, DBInfo dbinfo, Lobby[] lobbies, ThreadServerActiu servidorGeneral) throws IOException {
        // 0. Preparem les variables per al servidor deedicat en qüestió:
        this.dbinfo = dbinfo;
        this.usuari = new Usuari(); // esto no es el problema
    ...
    public void run() {
        ...
        try {
            ...
            while (!clientTancat) {
                ...
                System.out.println("\n\nHA ENTRAT AL BUCLE MENTRE EL CLIENT ESTIGUI OBERT PERO SESSIO TANCADA: espera a rebre l'ordre INICIAR_SESSIO o TANCAR");
                opcio = dinput.readUTF();
                System.out.println("ha llegit la OPCIO: "+opcio);
                this.doutput.writeBoolean(true);
                System.out.println("ha escrit el boolea al client");
                if (opcio.equals("INICIAR_SESSIO")) {
                    // chivato
                    System.out.println("Está en el IF de INICIAR_SESSIO!");
                    ...
                    if (sessioIniciada) {
                        sessioTancada = false;
                        System.out.println("Esperant a rebre l'ordre de l'usuari");
                        // 2. Espera a rebre l'ordre de l'usuari:
                        while (!sessioTancada) {
                            opcio = dinput.readUTF();
                            //System.out.println("Opcio rebuda: "+opcio);
                            switch (opcio) {
                                ...
                                case "V2":  // jugar vs2
                                    ...
                                    System.out.println("Ficarem l'usuari "+this.usuari.getNickname()+" a la cua vs2!");
                                    lobbies[0].addPlayer(usuari);
                                    break;

                                case "V4":  // jugar vs4
                                    doutput.writeBoolean(true);
                                    System.out.println("Ficarem l'usuari "+this.usuari.getNickname()+" a la cua vs4!");
                                    lobbies[1].addPlayer(usuari);
                                    break;

                                case "VT":  // jugar torneig
                                    doutput.writeBoolean(true);
                                    System.out.println("Ficarem l'usuari "+this.usuari.getNickname()+" a la cua de torneig!");
                                ...
                                case "TANCAR_SESSIO":  // tancar sessió
                                    doutput.writeBoolean(true);
                                    // CHIVATO
                                    System.out.println("HA ENTRADO EN EL CASE DE TANCAR_SESSIO");
                                    sessioTancada = true;
                                    clientTancat = false;
                                    break;
                                                ...
                                                for (int i = 0; i < lobbies[lobby].getCuaJugadors().size(); i++) {
                                                    if (lobbies[lobby].getCuaJugadors().get(i).getNickname().equals(rep[0])) {
                                                        if (rep[1].charAt(0) != '0') {
                                                            ...
                                                            System.out.println("Nova direcciÓ de " + lobbies[lobby].getCuaJugadors().get(i).getNickname() + ": " + lobbies[lobby].getCuaJugadors().get(i).getDireccion());
                                                        }
                    ...
                    System.out.println("NO HA REBUT EL MISSATGE D'INICIAR SESSIÓ. Opcio val: "+opcio);
                    clientTancat = true;
                }
            }
            ...
            System.out.println("ERROR NO ESPECIFICAT(ServerDedicat)!");
        }*/
    }
        ...
        for (int iLobby = 0; iLobby < 3; iLobby++) {
            for (int p = 0; p < lobbies[iLobby].getCuaJugadors().size(); p++) {
                if (lobbies[iLobby].getCuaJugadors().get(p).getNickname().equals(nomPlayer)) {
                    ...
                    System.out.println("Punts del jugador "+nomPlayer+": "+nPunts);
                }
            }
        }
    }
    ...
    private void treuJugadorCuaEspera() {
        ...
        // chivatoooooooo
        //if (found) System.out.println("HEM ESBORRAT L'USUARI DE LA CUA D'ESPERA");
    }
    ...
    private void modificaIEnviaControls() {
        try {
            ...
            // chivato
            System.out.println("Usuario actual conectado a este servidor: " + usuari.toString());
            ...
            while (!vistaTancada) {
                ...
                if (smsClient[0].equals("ENVIANT")) {
                    ...
                    //chivato
                    System.out.println("SMS rebut pel client: "+smsClient[0]+", "+smsClient[1]+", "+smsClient[2]+", "+smsClient[3]+"; ");
    ...
    private String[] enviaControlsUsuari(String[] controls) throws SQLException, IOException {
        ...
        // chivato
        System.out.println("Controls enviats exmple: primer control = "+controls[0]+"de l'usuari "+this.usuari);
    ...
    /**
     * Metode que esper a rebre un String del client
     * @return retorna l'String llegit
     * @throws IOException
     */
    public String repOrdre() throws IOException {
    ...
    /**
     * Metode que envia la grid de la partida on és cridat
     * @param grid grid de la partida a enviar
     * @throws IOException
     */
    public void enviaGrid(int[][] grid) throws IOException {
        this.ooutput.writeObject(grid);
    }
