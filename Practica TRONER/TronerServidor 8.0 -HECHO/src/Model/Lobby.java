 ...
 * per jugar una partida
 */
public class Lobby {    //todo faltaría ver como es el modo torneo/campeonato/como coño se llame
    ...
    private void comprobarCua() {
        try {
            ...
            // chivato muestra toda la lista de jugadores de esta lobby
            System.out.println("Usuaris en espera de " + this.nplayers + "jugadors: ");
            for (Usuari u : this.cuaJugadors) System.out.println(u.getNickname() + "; ");
            ...
            if (usuarisTrobats == nplayers) {
                ...
                // CHIVATO
                System.out.print("Usuaris de la partida: ");
                for (int j = 0; j < nplayers; j++) System.out.print(users[j].getNickname() + ", ");
                // Creem la partida
                creaPartida(users);
            }
        } catch (IOException e) {
    ...
    public void creaPartida(Usuari[] usuaris) throws IOException {
        ...
        //chivato
        System.out.println("VA A AVISAR A LOS USUARIOS");
        ...
        for (Usuari u : usersPartida) { //per cada usuari de la partida
            for (int j = 0; j < this.cuaJugadors.size(); j++) { // per cada jugador de la cua
                if (u.getNickname().equals(this.cuaJugadors.get(j).getNickname())) {    // si el nom coincideix amb el de la cua
                    for (int i = 0; i < this.serverGeneral.getLlistaServers().size(); i++) {
                        // chivato
                        //System.out.println("Enviando inicio de partida, registrando usuario: " + u.getNickname() + ";");
                        if (this.serverGeneral.getLlistaServers().get(i).getUsuari().getNickname().equals(u.getNickname())) {
                            this.serverGeneral.getLlistaServers().get(i).enviaBoolea(true);
                            //System.out.println("Enviando booleano al usuario "+u.getNickname());  // todo VIGILAR CON ESTE CHIVATO. IGUAL NO FUNCIONA POR ESTO.
