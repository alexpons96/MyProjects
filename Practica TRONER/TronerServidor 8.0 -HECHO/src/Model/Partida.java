...
/**
 * Classe que gestiona tots els procediments i execucions que succeeixen durant la partida
 */
public class Partida extends Thread {
        ...
        try {
            ...
            if (!torneig) {
                ...
                while (!alguExit) {  // mentre ningu hagi abandonat
                    ...
                    // VALOR DE VIVOS DE TODOS LOS JUGADORS (PLAYING())
                    for (Jugador j : jugadors) {
                        System.out.println(j.getUsuari().getNickname() + " isPlaying()=" + j.getUsuari().isPlaying());
                    }
                    ...
                    // MOSTREM LA GRID GENERADA PEL SERVIDOR:
                ...
                try {
                    ...
                    // chivato muestra todos los puntos al terminar la partida
                    for (int r = 0; r < njugadors; r++) {
                        System.out.println("Punts: " + jugadors[r].getUsuari().getPunts_partida());
                        System.out.println("Nombre de wins de cada jugador: "+jugadors[r].getUsuari().getWins());
                    }
    ...
    private void enviaWinner(boolean ultimaRonda) throws IOException, InterruptedException {
        ...
        if (ultimaRonda) {  // si es el fi de ronda, actualitzem el winner, si no el deixem a 5 i el player no actualitzara les victories
            //System.out.println("ultimaRonda="+ultimaRonda);
            if (empat) iGuanyador = 4;
            else {
                for (int q = 0; q < njugadors; q++) {
                    if (jugadors[q].getUsuari().isPlaying() && !empat) iGuanyador = q;
                    //System.out.println("GUANYADOR DE LA RONDA: "+jugadors[q].getUsuari().getNickname());
                }
            }
        }
    ...
    /**
     * Metode que espera a rebre un boolea del client (tot i que no el gestiona)
     * @throws IOException
     */
    private void esperaOK() throws IOException {
        for (Jugador j : jugadors) j.getServer().repBoolea();
    }
    ...
    private Jugador defineixServidorsDedicats(Jugador j) {
        for (ServerDedicat s: this.servidorGeneral.getLlistaServers()) {
    ...
    private void enviaUsuaris() throws IOException {
        for (Jugador u: jugadors) for (Jugador p:jugadors) {
            ...
            System.out.println("direccio de l'usuari: "+p.getUsuari().getDireccion());
        }
    }
